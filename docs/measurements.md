...
This file records what the detection suite (`tests/detection/`, raw Playwright with no library code) actually measures, next to the numbers from the spike: the prototype this library grew from, run in a single-CPU sandbox on Chrome 141. Every tolerance in the detection suite is justified here.
...
Both machines are much less noisy than the spike's single-CPU sandbox. The same work blocks about 7x longer on the runner than on the Mac (952ms against 134ms total blocking), which is why baselines must come from the machine that gates.
