...
- The result is written to `test-results/smoothness/<test>/auto.json` and attached as `smoothness: auto`, so the [reporter](../README.md#summarize-results-on-pull-requests) includes it.
...
- Runs are recorded on push builds of the main (or master) branch in CI: GitHub Actions, GitLab CI, Azure Pipelines and CircleCI are detected. Set `record: true` or `SMOOTHNESS_RECORD=1` to force it, or `record: false` to turn it off. Pull requests only compare, so they never change main's history.
...
When a spec file changes, the histories of the tests in it start again instead of failing: the test may now do different things. The whole spec file is hashed, so editing one test resets its neighbors too. That's conservative but simple. The result notes it, and the test gets a `smoothness-baseline-reset` annotation.
...
- As elsewhere, it only measures in Chromium. Other browsers get a `smoothness-skipped` annotation.
