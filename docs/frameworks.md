...
- **Inlining.** V8's optimizing compiler inlines small hot functions into their callers, and the sampled profile then attributes their time to the caller. In `examples/react-list`, `expensiveFormat()` is inlined into the `Row` component, so the profile names `Row` (at `main.jsx:18`, through the source map). DevTools shows the same.
