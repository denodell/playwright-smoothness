...
## Decisions made during M7

44. **Version 1.0.0**, as the release plan says, through a major changeset. `CHANGELOG.md` was generated by `changeset version` and then rewritten by hand into one readable 1.0 section, because the generated entries were ordered by commit hash and two were out of date (the "first preview" wording, and touch described as a compositor gesture).
45. **The preview's `next`-tag setup is gone**: `npm run release` builds and runs `changeset publish`, which publishes to `latest`. Publishing stays with you (it needs your npm login); `docs/releasing.md` has the steps.
46. **Every public option is tested.** The M7 audit found four untested ones (the reporter's `outputFile`, `title` and `githubSummary`, and automatic mode's `historyDir` and explicit `record`) and added tests for them.
47. **Interaction attribution is decided per script, not per frame.** Each LoAF script's `startTime` is recorded. A frame is an interaction frame if its input was waiting when it started, or if some script ran after the input arrived. Within it, a non-listener script that started before the input (a timer the click interrupted) isn't blamed, and its share of the frame is dropped. My first fix for the M6 race was frame-level ("`firstUIEventTimestamp` at the frame's start"), and CI showed it misclassified a real click frame whose input arrived mid-frame.
48. **`setInterval` callbacks are never an interaction's work.** Under 4x throttling a click's window is long, and a 250ms interval timer can fire inside it (between the handler and the paint). The brief's overlap rule then counted it, which made the "page that never goes quiet" test fail now and then on CI. An interval callback is periodic by construction, and work a handler triggers uses `setTimeout`, `requestAnimationFrame` or promises, so a frame made only of interval callbacks is load or background work, and interval scripts are never blamed. Checked with a 20-run repeat of the flaky test on the CI runner.

## Replays (folded into 1.0.0)

49. **Video replays of `scroll()`** (your request after M7; folded into the unpublished 1.0.0 at your request, so no 1.1.0 changeset). They're built from the frames full mode already records, so they don't change the measurement. They're encoded with WebCodecs VP8 in a throwaway secure page, and wrapped in a WebM written by the library, with cues for seeking. Attached by default only when a check got worse (`replay: 'on-regression'`). Playwright's built-in video was considered and not used for this, because it screencasts during the measurement.
