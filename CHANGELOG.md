...
- **`smoothness.scroll(locator, options)`** for lists: wheel (compositor-driven gestures), touch (real touch-event flicks; needs `hasTouch`), or arrow keys, vertically or horizontally, at named or numeric speeds, to a pixel distance or the end of the list (at most 20,000px). In full mode it detects **blank frames** from the trace's screenshots (`list.blankFramePercent`, gated, and `list.leastDrawnPercent`), with `list.background` and `list.placeholders` to say what counts as blank.
